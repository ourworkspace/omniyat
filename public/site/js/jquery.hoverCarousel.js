...
// By Yair Even-Or
// https://github.com/yairEO/hover-hcarousel
// http://dropthebit.com

;(function($){
    "use strict";

    var bindToClass      = 'hcarousel',
        containerWidth   = 0,
        scrollWidth      = 0,
        posFromLeft      = 0,    // Stripe position from the left of the screen
        stripePos        = 0,    // When relative mouse position inside the thumbs stripe
        animated         = null,
        $indicator, $hcarousel, el, $el, ratio, scrollPos, nextMore, prevMore, pos, padding;
    ...
    function move(e){
        ...
        stripePos = e.pageX - padding - posFromLeft; // the mouse X position, "normalized" to the hcarousel position

        if( stripePos < 0)
            stripePos = 0;

        pos = stripePos / (containerWidth - padding*2); // calculated position between 0 to 1
        // calculate the percentage of the mouse position within the hcarousel
        scrollPos = (scrollWidth - containerWidth ) * pos;   
        ...
        $hcarousel[prevMore ? "addClass" : "removeClass"]('left');
        $hcarousel[nextMore ? "addClass" : "removeClass"]('right');
    }

    $.fn.hcarousel = function(options){
        $(document)
            .on('mouseenter.hcarousel', '.' + bindToClass, calc)
            .on('mousemove.hcarousel', '.' + bindToClass, move);
    };
