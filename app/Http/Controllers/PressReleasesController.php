...
class PressReleasesController extends Controller
{
    ...
    {
        //print_r(date('d-m-Y',strtotime($request->date)));exit();
        request()->validate([
            'title'        =>   'required',
        ...
        try {
            ...
            $pressReleases = PressReleases::create([ 'title'=>$request->title, 'short_description'=>$request->ShortDescription,'long_description'=>$request->longDescription, 'thumb_image'=>$uploadThumbImage,'large_image'=>$uploadLargeImage, 'pdf_file'=>$uploadpdfFile,'date'=>date('d-m-Y',strtotime($request->date))])->id;
            if($pressReleases > 0):
    ...
    {
        $PageSub = PageSubTitles::where('page_type', 2)->first();
        $pressReleases = PressReleases::where('status',1)->orderBy('id','DESC')->get();
        return view('press_releases.press_release_list', compact('pressReleases','PageSub'));
    }
    ...
    public function pressReleaseUpdate(Request $request)
    {
        ...
        try {
            ...
            $pressReleases = PressReleases::where('id', $pressRelease->id)->update([ 'title'=>$request->title, 'short_description'=>$request->ShortDescription,'long_description'=>$request->longDescription, 'thumb_image'=>$uploadThumbImage,'large_image'=>$uploadLargeImage, 'pdf_file'=>$uploadpdfFile,'date'=>date('d-m-Y',strtotime($request->date))]);
            if($pressReleases > 0):
